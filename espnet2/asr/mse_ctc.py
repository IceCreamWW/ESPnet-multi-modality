...
class MSE_CTC(torch.nn.Module):
    ...
    def __init__(
        ...
        self.ctc_lo = torch.nn.Parameter(torch.FloatTensor(odim, encoder_output_size))
    ...
    def forward(self, hs_pad, hlens, ys_pad, ys_lens):
        ...
        ys_hat = -torch.cdist(self.dropout(hs_pad), self.ctc_lo) ** 2
        # ys_hat: (B, L, D) -> (L, B, D)
    ...
    def log_softmax(self, hs_pad):
        ...
        return F.log_softmax(torch.cdist(hs_pad, self.ctc_lo) ** 2, dim=2)
    ...
    def argmax(self, hs_pad):
        ...
        """
        return torch.argmax(torch.cdist(hs_pad, self.ctc_lo), dim=2)
