...
from espnet2.main_funcs.collect_stats import collect_stats_multi_iter
...
from espnet2.utils.types import str2tuple_str
...
class AbsTask(ABC):
    ...
    def get_parser(cls) -> config_argparse.ArgumentParser:
        ...
        group.add_argument(
            ...
            default=dict(),
            help="The keyword arguments for loaders",
        )
        group.add_argument("--train_shape_file_and_loader", type=str2tuple_str, action="append", default=[])
        group.add_argument("--valid_shape_file_and_loader", type=str2tuple_str, action="append", default=[])
        ...
        group.add_argument("--fold_length_and_loader", type=str2tuple_str, action="append", default=[])
        group.add_argument(
            "--sort_in_batch",
            type=str,
            default="descending",
        ...
        group.add_argument(
            "--train_data_path_and_name_and_type_and_loader",
            type=str2tuple_str,
            action="append",
            default=[],
        ...
        group.add_argument(
            "--valid_data_path_and_name_and_type_and_loader",
            type=str2tuple_str,
            action="append",
            default=[],
        )
    ...
    def main_worker(cls, args: argparse.Namespace):
        ...
        if args.dry_run:
            ...
            collect_stats_multi_iter(
                model=model,
                name2train_iter= {name: cls.build_streaming_iterator(
                        data_path_and_name_and_type=[t[:3] for t in args.train_data_path_and_name_and_type_and_loader if t[-1] == name],
                        key_file=next(t[0] for t in args.train_shape_file_and_loader if t[-1] == name),
                        batch_size=args.loaders[name]["batch_size"],
                    ...
                    ) for name in set(t[-1] for t in args.train_data_path_and_name_and_type_and_loader)
                },
                name2valid_iter= {name: cls.build_streaming_iterator(
                        data_path_and_name_and_type=[t[:3] for t in args.valid_data_path_and_name_and_type_and_loader if t[-1] == name],
                        key_file=next(t[0] for t in args.valid_shape_file_and_loader if t[-1] == name),
                        batch_size=args.loaders[name]["batch_size"],
                        dtype=args.train_dtype,
                        num_workers=args.num_workers,
                        allow_variable_data_keys=args.allow_variable_data_keys,
                        ...
                        preprocess_fn=cls.build_preprocess_fn(args, train=False),
                        collate_fn=cls.build_collate_fn(args, train=False),
                    ) for name in set(t[-1] for t in args.valid_data_path_and_name_and_type_and_loader)
                },
...
#                 )
                ...
                name2train_iter_factory = cls.build_iter_factories(
                    args=args,
                    distributed_option=distributed_option,
                    mode="train",
                )
            name2valid_iter_factory = cls.build_iter_factories(
                args=args,
                distributed_option=distributed_option,
                mode="valid",
            )
            if args.num_att_plot != 0:
                name2plot_attention_iter_factory = cls.build_iter_factories(
                    args=args,
                    distributed_option=distributed_option,
                    mode="plot_att",
                )
            else:
                name2plot_attention_iter_factory = None
            ...
            name2repeat = {name:args.loaders[name].get("repeat", 1) for name in args.loaders}
            cls.trainer.run(
                model=model,
                optimizers=optimizers,
                schedulers=schedulers,
                name2train_iter_factory=name2train_iter_factory,
                name2valid_iter_factory=name2valid_iter_factory,
                name2plot_attention_iter_factory=name2plot_attention_iter_factory,
                name2repeat=name2repeat,
    ...
    @classmethod
    def build_iter_factories(
        cls,
        args: argparse.Namespace,
        distributed_option: DistributedOption,
        mode: str,
        kwargs: dict = None,
    ) -> Dict[str, AbsIterFactory]:
        name2iter_factory = {}
        for name, loader_conf in args.loaders.items():
            args.batch_size = loader_conf.get("batch_size", 0)
            args.batch_bins = loader_conf.get("batch_bins", 0)
            args.valid_batch_size = loader_conf.get("batch_size", 0)
            args.valid_batch_bins = loader_conf.get("batch_bins", 0)
            args.fold_length = [int(t[0]) for t in args.fold_length_and_loader if t[-1] == name]
            if mode == "train":
                data_path_and_name_and_type = args.train_data_path_and_name_and_type \
                                            = tuple(t[:3] for t in args.train_data_path_and_name_and_type_and_loader if t[-1] == name)
                args.train_shape_file = [t[0] for t in args.train_shape_file_and_loader if t[-1] == name]
            else:
                data_path_and_name_and_type = args.valid_data_path_and_name_and_type \
                                            = tuple(t[:3] for t in args.valid_data_path_and_name_and_type_and_loader if t[-1] == name)
                args.valid_shape_file = [t[0] for t in args.valid_shape_file_and_loader if t[-1] == name]
            if data_path_and_name_and_type:
                iter_factory = cls.build_iter_factory(args, distributed_option, mode, kwargs)
                name2iter_factory[name] = iter_factory
        return name2iter_factory
