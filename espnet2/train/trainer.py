        ...
        for iepoch in range(start_epoch, trainer_options.max_epoch + 1):
            ...
            if not distributed_option.distributed or distributed_option.dist_rank == 0:
                ...
                try:
                    ...
                    logging.info("symlink not support, use shutil.copy")
                    shutil.copy(output_dir / f"{iepoch}epoch.pth", p)
...
class AlternateTrainer(Trainer):
    """Trainer having a optimizer.
    ...
    def run(
        ...
        optimizers: Sequence[torch.optim.Optimizer],
        schedulers: Sequence[Optional[AbsScheduler]],
        name2train_iter_factory: Dict[str, AbsIterFactory],
        name2valid_iter_factory: Dict[str, AbsIterFactory],
        name2plot_attention_iter_factory: Optional[Dict[str, AbsIterFactory]],
        name2repeat: Dict[str, int],
        trainer_options,
        distributed_option: DistributedOption,
    ) -> None:
        """Perform training. This method performs the main process of training."""
        ...
        for iepoch in range(start_epoch, trainer_options.max_epoch + 1):
            ...
            with reporter.observe("train") as sub_reporter:
                all_steps_are_invalid = cls.train_one_epoch(
                    ...
                    optimizers=optimizers,
                    schedulers=schedulers,
                    name2iterator={name:train_iter_factory.build_iter(iepoch) for name, train_iter_factory in name2train_iter_factory.items()},
            ...
            with reporter.observe("valid") as sub_reporter:
                cls.validate_one_epoch(
                    model=dp_model,
                    name2iterator={name:valid_iter_factory.build_iter(iepoch) for name, valid_iter_factory in name2valid_iter_factory.items()},
                    name2repeat=name2repeat,
                    reporter=sub_reporter,
                    options=trainer_options,
                    distributed_option=distributed_option,
            ...
            if not distributed_option.distributed or distributed_option.dist_rank == 0:
                # att_plot doesn't support distributed
                if name2plot_attention_iter_factory is not None:
                    with reporter.observe("att_plot") as sub_reporter:
                        cls.plot_attention(
                            model=model,
                            output_dir=output_dir / "att_ws",
                            summary_writer=summary_writer,
                            name2iterator={name:plot_attention_iter_factory.build_iter(iepoch) for name, plot_attention_iter_factory in name2plot_attention_iter_factory.items()},
                            reporter=sub_reporter,
                            options=trainer_options,
                        )
    ...
    def train_one_epoch(
        cls,
        model: torch.nn.Module,
        name2iterator: Dict[str,Iterable[Tuple[List[str], Dict[str, torch.Tensor]]]],
        name2repeat: Dict[str, int],
        optimizers: Sequence[torch.optim.Optimizer],
        schedulers: Sequence[Optional[AbsScheduler]],
        scaler: Optional[GradScaler],
        ...
        iterator = cls.make_multi_loader_iterator(name2iterator, name2repeat)
        log_interval = 100
        ...
        for iiter, ((_, batch), _type) in enumerate(
            reporter.measure_iter_time(iterator, "iter_time"), 1
        ):
            ...
            with autocast(scaler is not None):
                with reporter.measure_time("forward_time"):
                    retval = model(_type=_type, **batch)
    ...
    def plot_attention(
        ...
        model.eval()
        iterator = cls.make_multi_loader_iterator(name2iterator)
        for (ids, batch), _type in iterator:
            assert isinstance(batch, dict), type(batch)
            assert len(next(iter(batch.values()))) == len(ids), (
                len(next(iter(batch.values()))),
                len(ids),
    ...
    @classmethod
    def make_multi_loader_iterator(cls, name2iterator, name2repeat=None):
        name2iterator = {name:iter(iterator) for name, iterator in name2iterator.items()}
        try:
            for name, iterator in cycle(name2iterator.items()):
                if name2repeat is not None:
                    for i in range(name2repeat[name]):
                        yield next(name2iterator[name]), name
                else:
                    yield next(name2iterator[name]), name
