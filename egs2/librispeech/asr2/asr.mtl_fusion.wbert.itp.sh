...
token_type_in_para=raw      # Tokenization type (char or bpe).
...
phntoken_list="${token_listdir}"/phn/tokens.kaldi.txt
token_expand_stats="${token_listdir}"/phn/token_expand_stats.kaldi
...
if [ -z "${asr_stats_dir}" ]; then
    if [ ! -z "${asr_stats_tag}" ]; then
        asr_stats_dir="${expdir}/asr_stats_mtl_fusion_wbert_${train_set}_${asr_stats_tag}"
    else
        if [ "${lang}" != noinfo ]; then
            asr_stats_dir="${expdir}/asr_stats_mtl_fusion_wbert_${train_set}_fusion_${feats_type}_${lang}_${token_type}"
        else
            asr_stats_dir="${expdir}/asr_stats_mtl_fusion_wbert_${train_set}_fusion${feats_type}_${token_type}"
        fi
        if [ "${token_type}" = bpe ]; then
            asr_stats_dir+="${nbpe}"
        fi
...
# asr_stats_dir=/blob/tsst/users/v-weiwang1/espnet/egs2/librispeech/asr2/exp/debug_bert_stats
...
if ! "${skip_train}"; then
    ...
    if [ ${stage} -le 10 ] && [ ${stop_stage} -ge 10 ]; then
        ...
        # shellcheck disable=SC2086
        ${train_cmd} JOB=1:"${_nj}" "${_logdir}"/stats.JOB.log \
            ${python} -m espnet2.bin.asr_mtl_fusion_train \
                --collect_stats true \
                --use_preprocessor true \
                --bpemodel "${bpemodel}" \
                --token_type "${token_type}" \
                --token_type_in "${token_type_in}" \
                --token_type_in_para "${token_type_in_para}" \
                ...
                --train_data_path_and_name_and_type_and_loader "${_asr_train_dir}/alignment,alignment,text_int,speech" \
                --train_data_path_and_name_and_type_and_loader "${_asr_train_dir}/text_in_para,text_in_para,text,speech" \
...
#                 ${_opts} ${asr_args} || { cat "${_logdir}"/stats.1.log; exit 1; }
        ...
        <"${asr_stats_dir}/train/text_in_para_shape" \
            awk -v N="$(<${token_list_in} wc -l)" '{ print $0 "," N }' \
            >"${asr_stats_dir}/train/text_in_para_shape.${token_type_in_para}"
    ...
    if [ ${stage} -le 11 ] && [ ${stop_stage} -ge 11 ]; then
        ...
        if [ "${num_splits_asr}" -gt 1 ]; then
            ...
            _opts+="--train_data_path_and_name_and_type_and_loader ${_asr_train_dir}/alignment,alignment,text_int,speech "
            _opts+="--train_data_path_and_name_and_type_and_loader ${_asr_train_dir}/text_in_para,text_in_para,text,speech "
            ...
            _opts+="--train_shape_file_and_loader ${asr_stats_dir}/train/text_shape,speech "
            _opts+="--train_shape_file_and_loader ${asr_stats_dir}/train/alignment_shape,speech "
            _opts+="--train_shape_file_and_loader ${asr_stats_dir}/train/text_in_para_shape.${token_type_in_para},speech "
            _opts+="--train_shape_file_and_loader ${asr_stats_dir}/train/text_in_shape.${token_type_in},text "
...
if ! "${skip_eval}"; then
    if [ ${stage} -le 12 ] && [ ${stop_stage} -ge 12 ]; then
        ...
        for dset in ${test_sets}; do
            ...
            if "${use_k2}"; then
              ...
              asr_inference_tool="espnet2.bin.asr_mtl_fusion_inference"
            fi
            ...
            ${_cmd} --gpu "${_ngpu}" JOB=1:"${_nj}" "${_logdir}"/asr_inference.JOB.log \
                CUDA_VISIBLE_DEVICES=${decode_gpu} ${python} -m ${asr_inference_tool} \
                    --ngpu "${_ngpu}" \
    ...
    if [ ${stage} -le 13 ] && [ ${stop_stage} -ge 13 ]; then
        ...
        [ -f local/score.sh ] && local/score.sh ${local_score_opts} "${asr_exp}" "${dset}" "${inference_tag}"
